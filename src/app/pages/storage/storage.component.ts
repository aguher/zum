...
export class StorageComponent implements OnInit {
  ...
  emptyMsg: string =
    "No hay facturas añadidas. Añade tu primera factura en el formulario anterior";
  model: any = {
    billingsName: "",
    accountNumber: "",
    parent_account: "",
...
; // "<%= value %>"; */
      ...
      if (response.status !== "error") {
        response.articles.forEach((element) => {
          this.dataCombos.articles.push({
            label: element.code,
            value: element.id,
            family: element.brand,
          });
          this.dataCombos.articlesdesc.push({
            label: element.brand + " - " + element.description,
            value: element.id,
          });
        });
  ...
  getMovements() {
    this._api
      .getArticlesMovement(this.start_date, this.end_date)
      .subscribe((response) => {
        if (response !== null) {
          if (response.error) {
            this._auth.logout();
  ...
  parseDeleted(response) {
    if (response.status === "ok") {
      ...
      this._notification.success(
        "¡Éxito!",
        "Se ha eliminado la línea de almacén"
      );
      this.getMovements();
      this.getStock();
    } else {
  ...
  parseDeletedExportacion(response) {
    if (response.status === "ok") {
      ...
      this.exportaciones.splice(idx, 1);
      this.exportaciones = [...this.exportaciones];
      this.displayDialogDeleteExportation = false;
      this._notification.success("¡Éxito!", "Se ha eliminado la exportación");
    } else {
      this.displayDialogDeleteExportation = false;
      this._notification.error("¡Error!", response.msg);
    }
  ...
  parseCreatedExportation(response) {
    if (response.status === "ok") {
      this._notification.success(
        "¡Éxito!",
        "Se ha creado la exportación número " + response.number
      );
  ...
  filtrar() {
    ...
    let filtroalm: string =
      this.filtroalmacen == undefined ? "" : this.filtroalmacen;
    let filtropas: string =
      this.filtropasillo == undefined ? "" : this.filtropasillo;
    let filtrosec: string =
      this.filtroseccion == undefined ? "" : this.filtroseccion;
    let filtroalt: string =
      this.filtroaltura == undefined ? "" : this.filtroaltura;
    let filtroart: string =
      this.filtroartcod == undefined ? "" : this.filtroartcod;
    let filtronom: string =
      this.filtronombre == undefined ? "" : this.filtronombre;
    ...
    let filtrocad: string =
      this.filtrocaducidad == undefined ? "" : this.filtrocaducidad;
    let filtroemb: string =
      this.filtroembalaje == undefined ? "" : this.filtroembalaje;
    let filtrocan: string =
      this.filtrocantidad == undefined ? "" : this.filtrocantidad;
    let filtrouxe: string =
      this.filtrounidadesxe == undefined ? "" : this.filtrounidadesxe;
    let filtrotot: string =
      this.filtrototal == undefined ? "" : this.filtrototal;
    ...
    for (let i = 0; i < this.billings.length; i++) {
      if (
        this._common
          .getCleanedString(this.billings[i].almacen)
          .indexOf(this._common.getCleanedString(filtroalm)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].pasillo)
          .indexOf(this._common.getCleanedString(filtropas)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].seccion)
          .indexOf(this._common.getCleanedString(filtrosec)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].altura)
          .indexOf(this._common.getCleanedString(filtroalt)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].codigo)
          .indexOf(this._common.getCleanedString(filtroart)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].articulo)
          .indexOf(this._common.getCleanedString(filtronom)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].familia)
          .indexOf(this._common.getCleanedString(filtrofam)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].cliente)
          .indexOf(this._common.getCleanedString(filtrocli)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].marca)
          .indexOf(this._common.getCleanedString(filtromar)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].caducidad)
          .indexOf(this._common.getCleanedString(filtrocad)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].embalaje)
          .indexOf(this._common.getCleanedString(filtroemb)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].unidades)
          .indexOf(this._common.getCleanedString(filtrocan)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].unidadesxitem)
          .indexOf(this._common.getCleanedString(filtrouxe)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].total)
          .indexOf(this._common.getCleanedString(filtrotot)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].state)
  ...
  filtrarmov() {
    ...
    let filtromovfec: string =
      this.filtromovfecha == undefined ? "" : this.filtromovfecha;
    let filtromovmov: string =
      this.filtromovmovimiento == undefined ? "" : this.filtromovmovimiento;
    let filtromovalm: string =
      this.filtromovalmacen == undefined ? "" : this.filtromovalmacen;
    let filtromovpas: string =
      this.filtromovpasillo == undefined ? "" : this.filtromovpasillo;
    let filtromovsec: string =
      this.filtromovseccion == undefined ? "" : this.filtromovseccion;
    let filtromovalt: string =
      this.filtromovaltura == undefined ? "" : this.filtromovaltura;
    let filtromovari: string =
      this.filtromovidart == undefined ? "" : this.filtromovidart;
    let filtromovdes: string =
      this.filtromovarticulo == undefined ? "" : this.filtromovarticulo;
    let filtromovuni: string =
      this.filtromovcantidad == undefined ? "" : this.filtromovcantidad;
    let filtromovped: string =
      this.filtromovpedido == undefined ? "" : this.filtromovpedido;
    let filtromovnic: string =
      this.filtromovusuario == undefined ? "" : this.filtromovusuario;
    let filtromovobs: string =
      this.filtromovmotivo == undefined ? "" : this.filtromovmotivo;

    for (let i = 0; i < this.exportaciones.length; i++) {
      if (
        this._common
          .getCleanedString(this.exportaciones[i].date)
          .indexOf(this._common.getCleanedString(filtromovfec)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].name)
          .indexOf(this._common.getCleanedString(filtromovmov)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].location_warehouse)
          .indexOf(this._common.getCleanedString(filtromovalm)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].location_row)
          .indexOf(this._common.getCleanedString(filtromovpas)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].location_section)
          .indexOf(this._common.getCleanedString(filtromovsec)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].location_height)
          .indexOf(this._common.getCleanedString(filtromovalt)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].article_id)
          .indexOf(this._common.getCleanedString(filtromovari)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].description)
          .indexOf(this._common.getCleanedString(filtromovdes)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].units)
          .indexOf(this._common.getCleanedString(filtromovuni)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].ped_code)
          .indexOf(this._common.getCleanedString(filtromovped)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].nickname)
          .indexOf(this._common.getCleanedString(filtromovnic)) >= 0 &&
        this._common
          .getCleanedString(this.exportaciones[i].observations)
          .indexOf(this._common.getCleanedString(filtromovobs)) >= 0
      ) {
  ...
  filtrarstock() {
    this.stockfiltrados = [];
    let filtrofec: string =
      this.filtrosckfecha == undefined ? "" : this.filtrosckfecha;
    let filtropcd: string =
      this.filtrosckpedcode == undefined ? "" : this.filtrosckpedcode;
    let filtroped: string =
      this.filtrosckpedido == undefined ? "" : this.filtrosckpedido;
    let filtrocli: string =
      this.filtrosckcliente == undefined ? "" : this.filtrosckcliente;
    let filtrouni: string =
      this.filtrosckunidades == undefined ? "" : this.filtrosckunidades;
    let filtrocod: string =
      this.filtrosckcodigo == undefined ? "" : this.filtrosckcodigo;
    let filtroart: string =
      this.filtrosckarticulo == undefined ? "" : this.filtrosckarticulo;

    for (let i = 0; i < this.stock.length; i++) {
      if (
        this._common
          .getCleanedString(this.stock[i].creation_date)
          .indexOf(this._common.getCleanedString(filtrofec)) >= 0 &&
        this._common
          .getCleanedString(this.stock[i].ped_code)
          .indexOf(this._common.getCleanedString(filtropcd)) >= 0 &&
        this._common
          .getCleanedString(this.stock[i].campaign_name)
          .indexOf(this._common.getCleanedString(filtroped)) >= 0 &&
        this._common
          .getCleanedString(this.stock[i].customer_name)
          .indexOf(this._common.getCleanedString(filtrocli)) >= 0 &&
        this.stock[i].units
          .toString()
          .indexOf(this._common.getCleanedString(filtrouni)) >= 0 &&
        this._common
          .getCleanedString(this.stock[i].code)
          .indexOf(this._common.getCleanedString(filtrocod)) >= 0 &&
        this._common
          .getCleanedString(this.stock[i].description)
          .indexOf(this._common.getCleanedString(filtroart)) >= 0
      ) {
  ...
  newfiltrarstock() {
    ...
    let filtroent: string =
      this.new_filtrosckfechaentrega == undefined
        ? ""
        : this.new_filtrosckfechaentrega;
    let filtrofec: string =
      this.new_filtrosckfecha == undefined ? "" : this.new_filtrosckfecha;
    let filtropcd: string =
      this.new_filtrosckpedcode == undefined ? "" : this.new_filtrosckpedcode;
    let filtroped: string =
      this.new_filtrosckpedido == undefined ? "" : this.new_filtrosckpedido;
    let filtrocli: string =
      this.new_filtrosckcliente == undefined ? "" : this.new_filtrosckcliente;
    let filtrouni: string =
      this.new_filtrosckunidades == undefined ? "" : this.new_filtrosckunidades;
    let filtrodis: string =
      this.new_filtrosckdisponibles == undefined
        ? ""
        : this.new_filtrosckdisponibles;
    let filtroobs: string =
      this.new_filtrosckobservaciones == undefined
        ? ""
        : this.new_filtrosckobservaciones;

    for (let i = 0; i < this.stocknuevos.length; i++) {
      if (this.stocknuevos[i].delivery_date == null)
        this.stocknuevos[i].delivery_date = "";
      if (
        this._common
          .getCleanedString(this.stocknuevos[i].delivery_date)
          .indexOf(this._common.getCleanedString(filtroent)) >= 0 &&
        this._common
          .getCleanedString(this.stocknuevos[i].creation_date)
          .indexOf(this._common.getCleanedString(filtrofec)) >= 0 &&
        this._common
          .getCleanedString(this.stocknuevos[i].ped_code)
          .indexOf(this._common.getCleanedString(filtropcd)) >= 0 &&
        this._common
          .getCleanedString(this.stocknuevos[i].campaign_name)
          .indexOf(this._common.getCleanedString(filtroped)) >= 0 &&
        this._common
          .getCleanedString(this.stocknuevos[i].customer_name)
          .indexOf(this._common.getCleanedString(filtrocli)) >= 0 &&
        this.stocknuevos[i].units
          .toString()
          .indexOf(this._common.getCleanedString(filtrouni)) >= 0 //&&
  ...
  tramitarsalida(itemstock) {
    ...
    let necesidadespedido = _.filter(this.stock, function (o) {
      return o.id_campaign == id_campaign;
    });
    this.selectedStock = itemstock;
    necesidadespedido.forEach((el) => {
      let restantes = +el.units;
      let presalidas = _.filter(this.billings, function (o) {
        return o.article_id == el.article_id;
      });
  ...
  tramitarsalidaapi() {
    this.salidasfiltradas.forEach((element) => {
      if (element.sacar && element.sacar > 0) {
        ...
        this._api.exitStorage(body).subscribe(function (response) {
          that.selectedStock = false;

          that.refresh(false);
          //deletear fila
          if (response.status == "ok") {
            that._notification.success(
              "¡Éxito!",
              "Salida realizada correctamente"
            );
          } else {
            that._notification.error("Error", response.msg);
          }
        });
      }
    });
    ...
    if (!dataSelected) {
      ...
      if (true) {
        ...
        if (this.validateStorage(idarticulo, idcliente) == true) {
          ...
          this._api.insertStorage(body).subscribe((response) => {
            if (response.status === "ok") {
              this.billings = [];
              this.billings = response.items;
              this.billings = [...this.billings];
              ...
              this._notification.success("¡Éxito!", "Se ha añadido la línea");
              this.getMovements();
              this.getStock();
            } else {
              this._notification.error("¡Error!", response.msg);
  ...
  updatecod(event) {
    if (event.which === 13 || event.which === 9 || event.which == undefined) {
      ...
      if (idx >= 0) {
        ...
        this.selectedStorage.articulo = this.selectedStorage.articulo;
      }
    }
  }
    ...
    if (this.roleUser != 10) {
      ...
      if (storage) {
        ...
        this.selectedStorage = {
          ...
          idestado:
            idestadodefault >= 0
              ? this.dataCombos.states[idestadodefault].value
              : idestadodefault,
          idalmacen:
            idalmacendefault >= 0
              ? this.dataCombos.warehouses[idalmacendefault].value
              : idalmacendefault,
          art: "",
          seccion: "",
          altura: "",
  ...
  ordenar(ev) {
    if (ev.srcElement) {
      ...
      if (
        ...
        campoord == "seccion" ||
        campoord == "total" ||
        campoord == "unidadesxitem" ||
        campoord == "unidades"
      ) {
        this.facturasfiltradas = _.orderBy(
          this.facturasfiltradas,
  ...
  ordenar_sck(ev) {
    if (ev.srcElement) {
      ...
      if (campoord == "units" || campoord == "disponibles") {
        this.stockfiltrados = _.orderBy(
          this.stockfiltrados,
          [(item) => parseFloat(item[this.sck_orden])],
          [this.sck_ordsentido ? "desc" : "asc"]
        );
      } else {
        this.stockfiltrados = _.orderBy(
          this.stockfiltrados,
          [
            (item) =>
              item[this.sck_orden] == null
                ? ""
                : item[this.sck_orden].toLowerCase(),
          ],
          [this.sck_ordsentido ? "desc" : "asc"]
        );
      }
  ...
  salida() {
    ...
    if (isNaN(this.unidadessalida)) {
      this._notification.info(
        "¡Aviso!",
        "Debe introducir un número correcto de unidades"
      );
      return false;
    }
  ...
  parseUpdate(response) {
    if (response.status === "ok") {
      ...
      this._notification.success("¡Éxito!", "Se ha actualizado la línea");
      this.getBillsandExportations();
    } else {
      this.displayDialog = false;
      this._notification.error("¡Error!", response.msg);
  ...
  sendValuesChart(vec) {
    ...
    vec.mensual.forEach((element) => {
      ...
      while (mes != element.mes || anyo != element.anyo) {
        this.lineChartData[
          parseInt(element.anyo) - this.hoy.getFullYear() + 1
        ].data.push(0);
        mes++;
      }
      ...
      switch (type) {
        case "logo":
          if (data.file.type === "image/jpeg") {
            ...
            this._notification.error(
              "Error",
              "Solo están permitidos archivos JPG de imagen"
            );
          }
          break;
        case "erp":
          if (data.file.name.split(".")[1] === "xlsx") {
            ...
            this._notification.error(
              "Error",
              "Solo están permitidos archivos Excel"
            );
          }
          break;
        case "backup":
  ...
  getAlbaran(bImprimirTodo) {
    this._api
      .getInfoAlbaran(this.selectedStock.id_campaign)
      .subscribe((response) => {
        if (response !== null) {
          ...
          texto_lineas.forEach((subline) => {
            ...
            if (izqUnidades > 0 && pintaunidades) {
              if (bPintarCodigo) {
                ...
                doc.textEx(
                  this._common.getDecimals(line.amount, 2, true),
                  3.8,
                  altura_texto + 0.06,
                  "right",
                  "bottom"
  ...
  pintarComunesAlbaran(
    ...
    if (this.infoAlbaran.customer_city) {
      cliente.push(
        (this.infoAlbaran.customer_postal_code
          ? this.infoAlbaran.customer_postal_code + " "
          : " ") +
          (this.infoAlbaran.customer_city ? this.infoAlbaran.customer_city : "")
      );
    } else {
    ...
    if (
      this.infoAlbaran.customer_cif &&
      this.infoAlbaran.customer_cif != "undefined"
    )
      cliente.push("CIF: " + this.infoAlbaran.customer_cif);
    if (this.infoAlbaran.contact != "")
      cliente.push("Contacto: " + this.infoAlbaran.contact);
    if (this.infoAlbaran.phone != "" && this.infoAlbaran.phone != null)
      cliente.push("Teléfono: " + this.infoAlbaran.phone);
    if (this.infoAlbaran.project != "" && this.infoAlbaran.project != null)
      cliente.push("Campaña: " + this.infoAlbaran.project);
    if (this.infoAlbaran.phone2 != "" && this.infoAlbaran.phone2 != null)
      cliente.push("Teléfono: " + this.infoAlbaran.phone2);
    if (this.infoAlbaran.email != "" && this.infoAlbaran.email != null)
      cliente.push("E-Mail: " + this.infoAlbaran.email);
    if (
      this.infoAlbaran.solicitant_data != "" &&
      this.infoAlbaran.solicitant_data != null
    )
      cliente.push(
        "Datos del solicitante: " + this.infoAlbaran.solicitant_data
      );
