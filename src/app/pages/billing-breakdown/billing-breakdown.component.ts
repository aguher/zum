...
import * as imageToBase64Browser from "image-to-base64/browser";
...
@Component({
  selector: "billing-breakdown",
  templateUrl: "./billing-breakdown.component.html",
  styleUrls: ["./billing-breakdown.component.scss"],
})
export class BillingBreakdownComponent implements OnInit, OnDestroy {
    ...
    this.route.params.subscribe((params) => {
      ...
      this._api.getInfoBilling(`${this.idBill}`).subscribe((response) => {
        ...
        if (this.addresses.length > 0) {
          ...
          let direccion = {
            id: -1,
            city: this.infoBilling.address_bis
              ? this.infoBilling.address_bis
              : this.infoBilling.city,
            postal_code: this.infoBilling.address_bis
              ? ""
              : this.infoBilling.postal_code,
            address: this.infoBilling.address,
            Nombre: this.infoBilling.customer_name,
          };
          this.addresses.splice(0, 0, direccion);
          if (this.infoBilling.id_address != -1)
            this.changeAddress(this.infoBilling.id_address, false);
        }
      });
    });
  }
  ...
  changeAddress(valor, bSelect) {
    ...
    if (bSelect) {
      this._api
        .updateInfoBill("id_Address", valor, this.idBill)
        .subscribe((response) => {
  ...
  getLinesSubconcept() {
    this.lines = [];
    this._api.getSubconceptsBillings(this.idBill).subscribe((response) => {
      this.info =
        response.info_billing &&
        response.info_billing.length &&
        response.info_billing[0];
      ...
      let issueDate = this._common.getDateParsed(
          response.info_billing[0].issue_date
        ),
        dueDate = this._common.getDateParsed(response.info_billing[0].due_date);
  ...
  removeSubconcept(id, idx, id_inner) {
    this._api.removeSubconceptBilling(id).subscribe((response) => {
  ...
  calculateTotals() {
    ...
    this.total.budget_total =
      ...
      (parseFloat(this.selectedTax) / 100 + 1);

    this.tax_base =
      this._common.checkNumber(this.fee.budget.total) +
      this.subtotal.budget_total;
  }
  ...
  changeInputFee(fee, event, field) {
    if (event.which === 13 || event.which === 9) {
      ...
      this.updateFee(fee, event, field, "bill", this.idBill).then(
        (response) => {
  ...
  changeTaxes(e) {
    this._api.updateTaxBilling(e.value, this.idBill).subscribe((response) => {
      if (response.status === "ok") {
        ...
        this._notification.error(
          "Error",
          "No se ha podido actualizar el porcentaje"
        );
      }
    });
  }
  ...
  onDateChanged(event: IMyDateModel, field) {
    ...
    this.tmpField = field;
    let yearSelected = JSON.parse(
      localStorage.getItem("selectedFiscalYear")
    ).label;
    if (event.date.year !== parseInt(yearSelected)) {
      this._notification.error(
        "Error",
        `La fecha seleccionada debe estar dentro del año ${yearSelected}`
      );
    ...
    if (field === "issue_date") {
      ...
      cmpEnd = new Date(
        `${this.due_date.date.year}-${this.due_date.date.month}-${this.due_date.date.day}`
      );
    } else {
      cmpStart = new Date(
        `${this.issue_date.date.year}-${this.issue_date.date.month}-${this.issue_date.date.day}`
      );
  ...
  drawHeaderFooterPDF(
    ...
    doc.text(
      12,
      2.2,
      `Num. factura: ${this.serie}-${this._common.completeNumber(
        infoBilling.number
      )}`
    );
      ...
      doc.text(
        ...
        "Fecha de emisión: " +
          this._common.parseDatefromDate(
            ...
            }-${issueDate.getDate()}-${issueDate.getFullYear()}`
          )
      );
    }
  ...
  pintarComunesPaginas(
    ...
    if (this.infoBilling.due_date == "0000-00-00") {
      ...
      fechavto =
        this.infoBilling.due_date.split("-")[2] +
        "/" +
        this.infoBilling.due_date.split("-")[1] +
        "/" +
        this.infoBilling.due_date.split("-")[0];
    }
    ...
    doc.text(
      5.2,
      8,
      this.infoBilling.numbertext +
        (this.infoBilling.id_rect != 0
          ? " (abona Fra. " + this.infoBilling.numbertext_rect + ")"
          : "")
    );
    ...
    if (this.infoBilling.city) {
      cliente.push(
        (this.infoBilling.postal_code
          ? this.infoBilling.postal_code + " "
          : " ") + (this.infoBilling.city ? this.infoBilling.city : "")
      );
    } else {
    ...
    if (this.infoBilling.PO) {
      doc.textEx(
        `Su Referencia: ${this.infoBilling.PO}`,
        19,
        8.9,
        "right",
        "bottom"
    ...
    doc.textEx(
      `Ntra. Ref.: Pedido ${this.infoBilling.ped_code}`,
      19,
      9.5,
      "right",
      "bottom"
    ...
    if (this.infoBilling.numcampana) {
      doc.textEx(
        `Nº Campaña: ${this.infoBilling.numcampana}`,
        13,
        9.5,
        "right",
        "bottom"
 ...
 generatePDF(img){
  ...
  drawHeaderDurbanityBill(doc) {
    ...
    //empzamos con la cabecera de la factura
    doc.setFontSize(10);
    doc.setTextColor(227, 15, 95);
    doc.setFontType("bold");
    doc.text(12, 0.9, dataCompany.name);

    doc.setTextColor(0);
    doc.setFontSize(9);
    doc.setFontType("normal");

    doc.text(12, 1.5, dataCompany.address);
    doc.text(12, 2, dataCompany.address_bis);
    doc.text(12, 2.5, "Cif: " + dataCompany.cif);

    doc.text(17, 1.5, "T/Fax. 910561973");
    doc.text(17, 2, "info@durbanity.com");
    doc.text(17, 2.5, "www.durbanity.com");
    //fin de la cabecera
    ...
    doc.rect(11.7, 3.2, 8, 3.2);
    doc.setFillColor(227, 15, 95);
    doc.rect(11.9, 6.4, 7.9, 0.1, "F");
    doc.rect(19.7, 3.3, 0.1, 3.1, "F");

    doc.setTextColor(68, 84, 106);
    doc.setFontSize(12);
    doc.setFontType("bold");
    doc.text(11.9, 3.9, this.infoBilling.customer_name);
    doc.setFontType("normal");

    doc.setTextColor(0, 0, 0);
    doc.setFontSize(10);
    doc.text(11.9, 4.4, this.infoBilling.address);
    doc.text(
      11.9,
      4.9,
      this.infoBilling.Postal_Code + " " + this.infoBilling.city
    );
    doc.text(11.9, 5.4, this.infoBilling.city);
    doc.text(11.9, 5.9, "CIF: " + this.infoBilling.cif);
    ...
    doc.text(1.6, 8, "Nº de Factura:");
    doc.text(6, 8, "Fecha:");
    doc.text(9, 8, "Cod. Cliente");

    doc.setFontSize(11);
    doc.setTextColor(0);
    doc.setFontType("normal");
    let fecha = this.infoBilling.issue_date.split("-");
    doc.text(1.9, 9.1, this.infoBilling.numbertext);
    doc.text(5.5, 9.1, fecha[2] + "/" + fecha[1] + "/" + fecha[0]);
    doc.text(9, 9.1, this.infoBilling.id_customer);
    ...
    doc.setFontSize(11);
    doc.text(2, 11.6, "Descripción");
    doc.text(11.3, 11.6, "Cantidad");
    doc.text(14.7, 11.6, "P/U");
    doc.text(18, 11.6, "Total");

    doc.setDrawColor(227, 15, 95);
    doc.setLineWidth(0.01);
    doc.line(1.9, 12, 10, 12);
    doc.line(10.8, 12, 13.1, 12);
    doc.line(14, 12, 16.2, 12);
    doc.line(17, 12, 19.3, 12);
  }
  ...
  exportPDFDurbanity() {
    ...
    imageToBase64Browser(urlCompany) // Path to the image
      .then((response) => {
        ...
        doc.addImage(response, "PNG", 2, 1);
        this.drawHeaderDurbanityBill(doc);
        ...
        this.lines.forEach((line, idx) => {
          line.subconcepts.forEach((sub, otroidx) => {
            ...
            texto_lineas.forEach((line) => {
              ...
              doc.text(margen_izq, altura_texto, line);
              if (pintaunidades) {
                doc.textEx(
                  sub.budget.amount,
                  margen_izq + 10.8,
        ...
        doc.setTextColor(227, 15, 95);
        doc.text(2, 22, "Forma de pago ingreso en cuenta:");
        doc.text(2, 22.6, "IBAN: ES92 2100 3918 86 0200187324");
        doc.setFontSize(9);

        doc.text(
          1.1,
          24.5,
          "Observaciones: El cliente abonará las piezas no retornadas, o que esten deterioradas, segun la tabla de precios de reposicion de"
        );
        doc.text(1.1, 25, "DURBANITY.");

        doc.setFontSize(10);
        doc.setTextColor(68, 84, 106);
        doc.setFontType("bold");
        doc.text(13.5, 21.5, "Total Neto:");
        doc.text(14.8, 22.1, "IVA:");
        doc.text(13.7, 22.7, "Total IVA:");
        doc.text(14.5, 23.4, "Total:");

        let base = this.tax_base;
        let tax = base * (parseFloat(this.selectedTax) / 100);

        doc.setTextColor(0);
        doc.textEx(this._common.currencyFormatES(base), 18, 21.2, "right");
        doc.textEx(
        ...
        doc.textEx(this._common.currencyFormatES(tax), 18, 22.4, "right");
        doc.textEx(
        ...
        doc.setDrawColor(227, 15, 95);
        doc.setLineWidth(0.01);
        doc.line(1.9, 28.4, 19, 28.4);

        doc.rect(13.2, 20.8, 5.5, 2.8);
        ...
        console.log("info billing;", this.infoBilling);
