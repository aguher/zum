...
/*  files: UploadFile[];
  ...
  emptyMsg: string = "No hay subconceptos añadidos. Añade tu primer subconcepto en el formulario anterior";
  model: any = {
    subconceptName: '',
    id_customer: '',
    unit_price: '',
...
/*    this.files = []; // local uploading files array
    ...
    if (this.id_company == 414){
      this.nombresColumnas= ['parte','set','descripc. set','unids.por torno','proveedor','descripc. rem','perec.'];
    }else {
      this.nombresColumnas= ['familia','perecedero','almacenable','unids.por item','cliente','descripc.','set'];
    }
   }
  ...
  filtrar(){
    ...
    let filtrodes:string = (this.filtrodescripcion == undefined ? "" : this.filtrodescripcion);
    let filtropun:string = (this.filtropreciounitario == undefined ? "" : this.filtropreciounitario);
    let filtropit:string = (this.filtroudsporitem == undefined ? "" : this.filtroudsporitem);
    ...
    let filtroper:string = (this.filtroperecedero == undefined ? "": this.filtroperecedero);
    let filtrostc:string = (this.filtrostockable == undefined ? "": this.filtrostockable);
    let filtrocli:string = (this.filtrocliente == undefined ? "": this.filtrocliente);
    let filtrocon: string = (this.filtrocontacto == undefined ? "": this.filtrocontacto);


    for (let i= 0; i < this.subconcepts.length; i++){
      if (this._common.getCleanedString(this.subconcepts[i].code).indexOf(this._common.getCleanedString(filtrocod)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].description).indexOf(this._common.getCleanedString(filtrodes)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].unit_prices).indexOf(this._common.getCleanedString(filtropun)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].unitsperitem).indexOf(this._common.getCleanedString(filtropit)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].envelope).indexOf(this._common.getCleanedString(filtroemb)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].familyname).indexOf(this._common.getCleanedString(filtrofam)) >= 0 &&              
      this._common.getCleanedString(this.subconcepts[i].brand).indexOf(this._common.getCleanedString(filtromar)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].perishable).indexOf(this._common.getCleanedString(filtroper)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].stockable).indexOf(this._common.getCleanedString(filtrostc)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].customer_name).indexOf(this._common.getCleanedString(filtrocli)) >= 0 &&
      this._common.getCleanedString(this.subconcepts[i].contact).indexOf(this._common.getCleanedString(filtrocon)) >= 0 
        ){ 
...
/*     let eventosimulado:any= [];
  ...
  ordenar(ev){
    ...
    if (ev.srcElement){
      ...
      if (campoord == "unit_price" || campoord== "unitsperitem" ){
        this.articulosfiltrados =  _.orderBy(this.articulosfiltrados,[item => parseFloat(item[this.orden])],[(this.ordsentido ? 'desc' : 'asc')]);
      } else{
        this.articulosfiltrados =  _.orderBy(this.articulosfiltrados,[item => (item[this.orden] == null ? "" : item[this.orden].toLowerCase())],[(this.ordsentido ? 'desc' : 'asc')]);
      }
      this.numPage=1;
  }
  ...
  validateArticle(idfamilia, idembalaje, idcliente){
    if (isNaN(parseInt(this.selectedSubconcept.unit_price))  || isNaN(parseInt(this.selectedSubconcept.unitsperitem)) ) {
        this
        ._notification
      ...
      this
      ._notification
      .info("¡Aviso!", "Rellene el código del artículo");
      return false;
    }
    else if (this.selectedSubconcept.description == ""){
      this
      ._notification
      .info("¡Aviso!", "Rellene la descripción del artículo");
      return false;
    }
    else if (idfamilia < 0){
      this
      ._notification
      .info("¡Aviso!", "Debe seleccionar el campo " + this.nombresColumnas[0]);
      return false;
    }
    else if (idembalaje < 0){
      this
      ._notification
      .info("¡Aviso!", "Debe seleccionar el embalaje");
      return false;
    }
    else if (idcliente < 0){
      this
  ...
  addSubconcept() {    
    if (!this.id_company) {
      this._notification.error('¡Aviso!', 'Debes seleccionar una empresa desde el menu de selección.');
        ...
        body += '&id_family=' + (idfamilia == -1 ? -1 : this.dataCombos.families[idfamilia].value);
        body += '&envelope_type=' + (idembalaje == -1 ? -1 : this.dataCombos.envelopes[idembalaje].value);
        ...
        body += '&customer=' + (idcliente == -1 ? -1 : this.dataCombos.customer[idcliente].value);
        ...
        if (this.id_company == 414){
          ...
          body += '&descrip_provider=' + this.selectedSubconcept.descrip_provider;
        ...
        this._api.addSubconceptStandard(body).subscribe((response) => {
          if (response.status === 'ok') {
            this._notification.success('¡Éxito!', 'Se ha añadido el subconcepto');
...
updatePrecioCliente(preciocliente){
  this._api.updatecustomerPrice(`price=${preciocliente.unit_price}&id_customer=${preciocliente.id_customer}&id=${preciocliente.id}&id_article=${preciocliente.id_subconcept}`).subscribe((response) => {
    if (response !== null) {
      if (response.error) {
        this._auth.logout();
        this._router.navigate(['/login']);
  ...
  selectSubconcept(subconcept=null) {
    ...
    if (subconcept){
      ...
      this.selectedSubconcept = {
        ...
        unitsperitem:1,
        id_family:-1,
        envelope_type:(idenvelopedefault >= 0 ? this.dataCombos.envelopes[idenvelopedefault].value :idenvelopedefault),
        brand: '',
        bperishable: 0,
        bstockable: 1,
        customer: -1,
  ...
  updateApi(id) {
    if (!this.id_company) {
      ...
      this._api.updateSubconceptStandard(body).subscribe((response) => this.parseUpdate(response));
    }
    }
  }
  ...
  exportExcel(): void {
    ...
    saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), `listado_articulos.xlsx`);
  }
  ...
  parseUpdate(response) {
    if (response.status === 'ok') {
      ...
      this.cachedSubconcept.descrip_provider = response.items[0].descrip_provider;
  ...
  deleteApi(id) {
    this.idSelected = parseInt(this.selectedSubconcept.id, 10);
    this._api.deleteSubconceptStandard(this.idSelected).subscribe((response) => this.parseDeleted(response));
  }

  parseDeleted(response) {
    if (response.status === 'ok') {
      let idx = _.findIndex(this.subconcepts, (o) => { return o.id == this.idSelected; });
