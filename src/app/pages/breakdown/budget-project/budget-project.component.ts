...
@Component({ selector: 'budget-project', templateUrl: './budget-project.component.html', styleUrls: ['./budget-project.component.scss'] })
export class BudgetProjectComponent implements OnInit, OnDestroy {
  ...
  listProjects = null;
  listBudgets = null;
  constructor(private  _token: TokenService,private _api: ApiService, private _common: Common, private _config: Configuration, private _notification: NotificationsService) { 
  ...
  ngOnInit() {
    ...
    if (this.infoCustomer.customer.btramite != 0 || this.infoCustomer.customer.salidas != 0 || this.infoCustomer.customer.numfacturas != 0){
    ...
    let issueDate = this
    ._common
    .getDateParsed(this.infoCustomer.delivery_date);
    ...
    this._api.getProjectsBudgets().subscribe((response) => {
      ...
      response.projects.forEach(element => {
        ...
        if (parseInt(this.idProject) !== parseInt(element.id)) {
          this.listProjects.push({ label: element.campaign_name, value: element.id });
        } else {
      ...
      if (response !== null) {
        if (response.error) {
          ...
          if (this.bHayDirecciones){
            ...
            let direccion = {
              id: -1,
              city: (this.infoCustomer.customer.address_bis ? this.infoCustomer.customer.address_bis : this.infoCustomer.customer.city),
              postal_code: (this.infoCustomer.customer.address_bis ? '' : this.infoCustomer.customer.postal_code),
              address: this.infoCustomer.customer.address,
              Nombre: this.infoCustomer.customer.name 
            }
            this.addresses.splice(0,0,direccion);
            if (this.infoCustomer.id_address != -1) this.changeAddress(this.infoCustomer.id_address, false);
          }
        }
      }
    });
  ...
  changeAddress(valor, bSelect){
    ...
    if (bSelect){
      ...
      this
      ._api
      .updateInfoBudget("id_Address", valor, this.valueId).subscribe(response => {
        if(response.status === 'ok') {
          this._notification.success(this.infoCustomer.status, 'Se ha modificado la dirección.');
        } else {
      ...
      this
        ...
        .subscribe((response) => {
          if (response.status === 'ok') {
            this
              ._notification
              .success('Fecha de entrega', 'Se ha guardado satisfactoriamente.');
          }
        });
  ...
  changeInfoContact(event, field) {
    ...
    if (event.which === 13 || event.which === 9) {
      ...
      if (field=="contact"){
          ...
          this
            ...
            .subscribe((response) => {
              if (response.status === 'ok') {
                this
                  ._notification
                  .success('A la att.', 'Se ha guardado satisfactoriamente.');
                event.target.blur();
              }
            });
        }else{
          let cod;
          switch (field){
            case "project": cod = 1;break;
            case "phone": cod = 3;break;
            case "solicitant_data" : cod = 4; break;
          }
          this
          ._api
          .updateInfoPersonalFieldBudget(cod, 1, event.target.value, this.valueId)
          .subscribe((response) => {
            if (response.status === 'ok') {
              this
                ._notification
                .success(field, 'Se ha guardado satisfactoriamente.');
              event.target.blur();
            }
          });
        }
...
/*     if ((this.status_project === '1' || this.status_project === '2') && this._token.getInfo().role !== '4') {
  ...
  getLinesSubconcept() {
    ...
    this
      ._api
      .getSubconcept(this.valueId)
      .subscribe((response) => {
        this.info = response.info_project && response.info_project.length && response.info_project[0];

        this.start_date = response.info_project[0].start_date_budget;
        this.end_date = response.info_project[0].end_date_budget;
        let creationDate = this
          ._common
          .getDateParsed(response.info_project[0].start_date_budget),
          endDate = this
            ._common
            .getDateParsed(response.info_project[0].end_date_budget);
        ...
        if (response.subconcepts) {
          response
            ...
            .forEach((element) => {
              ...
              let total_real = element.amount * element.unit_budget * element.unit_real;
              let benefit = total_budget - total_real;
              let margin = (total_budget === 0)
                ? 0
                : (benefit / total_budget) * 100;
              ...
              varConcept.real.units += this
                ._common
                .toFloat(element.unit_real);
              varConcept.real.total += this
                ._common
                .toFloat(element.amount) * this
                  ._common
                  .toFloat(element.unit_budget) * this
              ...
              varConcept.others.margin = (varConcept.budget.total === 0
                ? 0
                : this._common.toFloat((varConcept.others.benefits / varConcept.budget.total) * 100));
              varConcept
                .subconcepts
                .push({
                  ...
                  real: {
                    units: this
                      ._common
                      .currencyFormatES(element.unit_real, false),
                    total: this
                      ._common
                      .currencyFormatES(total_real, false)
                  },
                  others: {
                    benefits: benefit,
                    margin: margin
        ...
        this.fee.real.units = this
          ._common
          .currencyFormatES(tmp.real.units, false);
        this.fee.real.total = 0;

        this.fee.others.benefits = this
          ._common
          .currencyFormatES(tmp.budget.total);
  ...
  updateSubconcept(subconcept, event, field, blurDescription = false) {
    ...
    if ((blurDescription && event.target) || (event && event.target && field !== 'name' && field !== 'code' ) || (!event.target && (field === 'name' || field === 'code') )) {
      ...
      if (description.length) {
       //console.log('price3',description[0]);
        ...
        subconcept.budget.price = (description[0]) ? this
          ._common
          .currencyFormatES(description[0].unit_price, false) : '0,00';
      ...
      if (code.length) {
        ...
        subconcept.budget.price = (code[index]) ? this
          ._common
          .currencyFormatES(code[index].unit_price, false) : '0,00';
      ...
      return new Promise((resolve) => {
        ...
        this
          ._api
          .updateSubconcept(subconcept.id, field, parsed, parsedPrice, parsedDescription,parsedCode)
          .subscribe((response) => {
            if (response.status == 'ok') {
              subconcept[field] = value;
              subconcept["name"] = (parsedDescription ?  parsedDescription: subconcept["name"]) ;
              ...
              if (this.id_company == 412 || this.id_company == 385){
                response.disponibles = (response.disponibles == null ? 0 : response.disponibles);
                response.solicitadas = (response.solicitadas == null ? 0 : response.solicitadas);

                if (response.bstockable == 1 && +response.solicitadas+parseInt(response.pedidasaqui) > +response.disponibles){
                  this.exceso = response;
                  let str = "Del artículo \"" + response.descripcion + "\"\n";
                  str+=  "Hay " + response.disponibles + " unidades en el almacén \n";
                  str+=  "Están comprometidas " + response.solicitadas + " en otros pedidos \n";
                  str+=  "Con lo cual tendríamos disponibles " + Math.max((+response.disponibles-parseInt(response.solicitadas)),0) + " unidades \n\n";
                  str+=  "Como usted pide " + response.pedidasaqui + " unidades, no se podría cumplir con las existencias actuales. Considere si debe cambiar las unidades.";
      ...
      .forEach(line => {
        ...
        line
          ...
          .forEach(subconcept => {
            ...
            line.real.units += this
              ._common
              .toFloat(subconcept.real.units);
            line.real.total += this
              ._common
              .toFloat(subconcept.real.total);

            line.others.benefits += this
              ._common
              .toFloat(subconcept.others.benefits);
            line.others.margin = 0;
            if (line.budget.total > 0) {
              line.others.margin = this._common.toFloat((line.others.benefits / line.budget.total) * 100);
            }
          });
  ...
  calculateTotals() {
      ...
      .forEach((items) => {
        items
          ...
          .forEach((inner) => {
            ...
            subtotal.real_total += this
              ._common
              .toFloat(inner.real.total);
            subtotal.benefit_total += this
              ._common
              .toFloat(inner.others.benefits);
          });
      });
    this.subtotal.margin_total = (subtotal.budget_total === 0)
      ? 0
    ...
    this.total.budget_total = subtotal.budget_total + this
      ._common
      .toFloat(this.fee.budget.total);
    this.total.real_total = subtotal.real_total + this
      ._common
      .toFloat(this.fee.real.total);
    this.total.benefit_total = subtotal.benefit_total + this
      ._common
      .toFloat(this.fee.others.benefits);
    this.total.margin_total = (this.total.budget_total === 0)
      ? 0
  ...
  changeInputFee(fee, event, field) {
    if (event.which === 13 || event.which === 9) {
      event.currentTarget.className = 'entertabeado';
      this
        .updateFee(fee, event, field, this.typeTable, this.numberId)
        .then((response) => {
      ...
      this
        ...
        .subscribe((response) => {
          if (response.status === 'ok') {
            this
              ._notification
              .success('Validez de presupuesto', 'Se ha guardado satisfactoriamente.');
            event.target.blur();
          }
        });
  ...
  drawHeaderFooterPDF(doc, imgData, imgDataCompany, dataCompany, infoCustomer, budget_validity, isBudget = true) {
    // HEADER
    doc.setFontSize(23);
    ...
    doc.text(12, row, 'Fecha: ' + this._common.parseDatefromDate(`${actualDate.getMonth() + 1}-${actualDate.getDate()}-${actualDate.getFullYear()}`));
    ...
    doc.text(1, 3.5, ccif);
    let caddress = infoCustomer.customer.address ? infoCustomer.customer.address : '';
    doc.text(1, 4, caddress);
    let caddressBis = infoCustomer.customer.address_bis ? infoCustomer.customer.address_bis : '';
...
//        if (parseInt(sub.budget.total) != 0){
          ...
          texto_lineas.forEach((line) => { 
            ...
            if (altura_texto > 19.5) {
              ...
              doc.textEx("Suma y sigue en página siguiente >>",sumaysiguederecha,20,'right','bottom');    
              doc.textEx(this._common.currencyFormatES(importeparcial,false), 18.8,20,'right','bottom');               
            ...
            switch (dataCompany.id){
              case "410": 
                if (pintaunidades) doc.textEx(this._common.currencyFormatES(sub.budget.amount,false),16.3,altura_texto+0.06,'right','bottom');                    
              ...
              case "411": 
                ...
                break;
              case "408": default:  
                if (pintaunidades) doc.textEx(this._common.currencyFormatES(sub.budget.amount,false),3.45,altura_texto+0.06,'right','bottom');                    
  ...
  if (this.infoCustomer.id_status=='1'){
    this.pintarRectanguloFactura(doc,inicio_cuadro+altura_cuadro+0.3,21-margen_dch-ancho_importes,margen_dch,0.6,2);
  }else{
    this.pintarRectanguloFactura(doc,inicio_cuadro+altura_cuadro+0.3,margen_izq,margen_dch,0.6,2);
    if (bPintarlineas){
      doc.line(21-margen_dch-ancho_importes,inicio_cuadro+altura_cuadro+0.3,21-margen_dch-ancho_importes,inicio_cuadro+altura_cuadro+2.3);
      doc.line(8.5,inicio_cuadro+altura_cuadro+0.3,8.5,inicio_cuadro+altura_cuadro+2.3);          
      doc.line(10.6,inicio_cuadro+altura_cuadro+0.3,10.6,inicio_cuadro+altura_cuadro+2.3);   
    }
  }

  //observaciones
  this.pintarRectanguloFactura(doc,inicio_cuadro+altura_cuadro+0.3+3,margen_izq,margen_dch,0.6,3.5);
  ...
  //resumen factura   
    if (this.infoCustomer.id_status=='1'){     
      ...
      doc.setTextColor(0);
      doc.text(10,inicio_cuadro+altura_cuadro+1.3,"(Impuestos no incluídos)");
      doc.text(10,inicio_cuadro+altura_cuadro+2.3, 'No incluye Portes');
      doc.textEx(this._common.currencyFormatES(this.total.budget_total),18.8,inicio_cuadro+altura_cuadro+1.3,'right','middle');
    }else{
      if (textoEnunciadoBlanco) doc.setTextColor(255);
      ...
      doc.text(9.1,inicio_cuadro+altura_cuadro+1.5,this._common.currencyFormatES(21,false) + '%' );
      doc.text(12.5,inicio_cuadro+altura_cuadro+1.5,this._common.currencyFormatES(tax));
      doc.textEx(this._common.currencyFormatES(this.total.budget_total*1.21),18.8,inicio_cuadro+altura_cuadro+1.3,'right','middle');
    }


    //observaciones
    if (textoEnunciadoBlanco) doc.setTextColor(255);    
    doc.text(margen_izq+0.2,inicio_cuadro+altura_cuadro+3.7,"Observaciones");    
    doc.setTextColor(0);
    altura_texto = 0; 
    ...
    texto_lineas.forEach((line) => {
      doc.text(margen_izq+0.2,inicio_cuadro+altura_cuadro+4.4+altura_texto,line);   
      altura_texto +=0.4;
    });
    ...
    doc.save(`${String(this.infoCustomer.status).trim()}-${String((this.infoCustomer.id_status=='1'?this.infoCustomer.campaign_code:this.infoCustomer.ped_code)).trim()}.pdf`);
    this
      ._notification
  ...
  pintarComunesPaginas(doc,margen_izq, margen_dch, inicio_cuadro, altura_cuadro,ancho_importes,borde_lineas){
    ...
    if (this.infoCustomer.customer.city){
      cliente.push((this.infoCustomer.customer.postal_code  ? this.infoCustomer.customer.postal_code + ' ' :' ') + (this.infoCustomer.customer.city ? this.infoCustomer.customer.city : ''));
    }else {
      cliente.push(this.infoCustomer.customer.address_bis);
    }

    if (this.infoCustomer.customer.cif && this.infoCustomer.customer.cif != "undefined") cliente.push('CIF: ' + this.infoCustomer.customer.cif);
    if (this.infoCustomer.contact != '') cliente.push('Contacto: ' + this.infoCustomer.contact);   
    if (this.infoCustomer.phone != '' && this.infoCustomer.phone!=null ) cliente.push('Teléfono: ' + this.infoCustomer.phone);      
    if (this.infoCustomer.delivery_date != '' && this.infoCustomer.delivery_date!=null ) cliente.push('Fecha Entrega: ' + this.delivery_date.date.day + "/" + this.delivery_date.date.month + "/"  + this.delivery_date.date.year );      
    if (this.infoCustomer.project != '' && this.infoCustomer.project!=null) cliente.push('Campaña: ' + this.infoCustomer.project);   
    if (this.infoCustomer.customer.phone != '' && this.infoCustomer.customer.phone!=null) cliente.push('Teléfono: ' + this.infoCustomer.customer.phone);   
    if (this.infoCustomer.customer.email != '' && this.infoCustomer.customer.email!=null) cliente.push('E-Mail: ' + this.infoCustomer.customer.email);   
    if (this.infoCustomer.solicitant_data != '' && this.infoCustomer.solicitant_data !=null) cliente.push('Datos del solicitante: ' + this.infoCustomer.solicitant_data);   
  ...
  pintarComunesAlbaran(doc,margen_izq, margen_dch, inicio_cuadro, altura_cuadro,ancho_importes,borde_lineas, bPintarCuadroPrincipal){
    ...
    //textos informativos de la factura
    doc.setFontType("bold");
    doc.text(2.2,7.8,"Albarán " + (this.infoCustomer.id_status=='1'?' Presupuesto nº ' + this.infoCustomer.campaign_code:'Pedido nº ' + this.infoCustomer.ped_code));

    if (bMostrarFechaEntrega && this.delivery_date){
      if (parseFloat(this.delivery_date.date.year) > 2000){
      doc.text(2.2,8.5,"Fecha de Entrega: " + this
      ._common
      .getFecha(new Date(this.delivery_date.date.year,this.delivery_date.date.month-1,this.delivery_date.date.day)));
      }
    }
          ...
          texto_lineas.forEach((line) => {
            ...
            if (izqUnidades > 0 && pintaunidades) {
              if (bPintarCodigo) {//si debo pintar código me tengo que ajustar
                doc.text(margen_izq+izqUnidades-1.5,altura_texto,sub.code); 
                doc.textEx(sub.budget.amount,3.8,altura_texto+0.06,'right','bottom');                    
              }else{
                doc.textEx(sub.budget.amount,4.3,altura_texto+0.06,'right','bottom'); 
              }
            }                   
            pintaunidades = false;
...
//      }
  ...
  if (pintarObservaciones) {
   ...
   texto_lineas.forEach((line) => {
    ...
    doc.save(`Albaran-${String((this.infoCustomer.id_status=='1'?this.infoCustomer.campaign_code:this.infoCustomer.ped_code)).trim()}.pdf`);
    this
      ._notification
      .success('Exportación', 'Se ha creado el archivo PDF.');
  ...
  exportExcel(): void {
    ...
    if (this.idProject) {
      saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), `presupuesto-cliente-${this.info.campaign_code}-${this.info.campaign_name}.xlsx`);
    } else if (this.idBudget) {
      saveAs(new Blob([s2ab(wbout)], { type: 'application/octet-stream' }), `presupuesto-cliente-${this.info.id}.xlsx`);
    }
  ...
  fillExcelData() {
    ...
    this.lines.forEach((line) => {
      exportData.push({
        ...
        'Precio Unitario': this._common.currencyFormatES(line.budget.price, false),
        'Total': this._common.currencyFormatES(line.budget.total, false),
        '': '',
        'Coste Unitario Real': this._common.currencyFormatES(line.real.units, false),
        ...
        'Margen': `${this._common.currencyFormatES(line.others.margin, false)}  %`,
      });
      if (line.subconcepts.length) {
        line.subconcepts.forEach(subconcept => {
          exportData.push({
            ...
            'Cantidad': this._common.currencyFormatES(subconcept.budget.amount, false),
            'Precio Unitario': this._common.currencyFormatES(subconcept.budget.price, false),
            'Total': this._common.currencyFormatES(subconcept.budget.total, false),
            '': '',
            'Coste Unitario Real': this._common.currencyFormatES(subconcept.real.units, false),
            'Total Real': this._common.currencyFormatES(subconcept.real.total, false),
            '  ': '',
            'Beneficio': this._common.currencyFormatES(subconcept.others.benefits, false),
            'Margen': `${this._common.currencyFormatES(subconcept.others.margin, false)} %`,
          });
        });
      }
    });
  /*   // subtotal
    ...
    exportData.push({
      ...
      'Margen': `${this._common.currencyFormatES(this.total.margin_total, false)} %`,
    });
    exportData.push({
      'Concepto': '',
      'Subconcepto': '',
  ...
  cloneBudget() {
    if (this.selectedSource) {
      this._api.cloneData(this.selectedSource, this.typeTable, this.numberId, this.typeSource).subscribe(response => {
        if (response.status === 'ok') {
          this.showClone = false;
          this.getLinesSubconcept();
          this._notification.success('Clonado', 'Se ha clonado satisfactoriamente el presupuesto');
          this.selectedSource = '';
        }
      })
