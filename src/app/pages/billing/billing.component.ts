...
; // "<%= value %>"; */
  ...
  public lineChartData: Array<any> = [
    { data: [], label: +this.fechahoy.getFullYear() - 2 },
    { data: [], label: +this.fechahoy.getFullYear() - 1 },
  ...
  refresh(mensaje: boolean = false) {
    ...
    this._api
      .getBills4Exportation(super_start_date, this.end_date)
      .subscribe((response) => {
        if (response !== null) {
          if (response.error) {
          } else if (response.status === "error") {
            ...
            this.allbillings = response.items.map((value) => {
              value.tax_base = this._common.currencyFormatES(
                value.tax_base,
                false
              );
  ...
  getBgColor(facturaSeleccionada) {
    if (facturaSeleccionada.charge_amount == "0") {
      ...
      this._common.getDecimals(facturaSeleccionada.charge_amount, 2, true) ==
      ...
      if (response !== null) {
        if (response.error) {
          ...
          response.items.forEach((element) => {
            ...
            if (
              element.charge_date !== "0000-00-00" &&
              element.charge_date !== null
            ) {
  ...
  createExportation() {
    ...
    this._api.createExportationBilling(billingsDate).subscribe((response) => {
  ...
  parseCobrada(response, numero) {
    if (response.status === "ok") {
      ...
      if (numero == 1) {
        this.billings[idx]["charge_amount"] = parseFloat(
          this.billings[idx]["total"]
            .toString()
            .replace(".", "")
            .replace(".", "")
            .replace(",", ".")
        ); //this._common.getFecha(this.hoy);
      ...
      this._notification.success("¡Éxito!", "Se ha realizado la operación");
    } else {
      this.displayDialogDelete = false;
  ...
  parseDeleted(response) {
    if (response.status === "ok") {
      ...
      this.filtrar();
      this.displayDialogDelete = false;
      this._notification.success("¡Éxito!", "Se ha eliminado la factura");
    } else {
      this.displayDialogDelete = false;
      this._notification.error("¡Error!", response.msg);
    }
  ...
  parseCreatedExportation(response) {
    if (response.status === "ok") {
      ...
      let fechafin = new Date(
        this.exportation_date.date.year,
        this.exportation_date.date.month - 1,
        this.exportation_date.date.day
      );
      this.allbillings.forEach((fact) => {
        ...
        if (fact.id_export == 0 && mifecha <= fechafin)
          fact.id_export = response.number;
      });
    } else {
      this._notification.error("¡Error!", response.msg);
    }
  ...
  recuperarfiltrosstorage() {
    this.filtronumero = localStorage.getItem(this.id_user + "FACfiltronumero")
      ? localStorage.getItem(this.id_user + "FACfiltronumero")
      : "";
    this.filtrodescripcion = localStorage.getItem(
      this.id_user + "FACfiltrodescripcion"
    )
      ? localStorage.getItem(this.id_user + "FACfiltrodescripcion")
      : "";
    this.filtrocliente = localStorage.getItem(this.id_user + "FACfiltrocliente")
      ? localStorage.getItem(this.id_user + "FACfiltrocliente")
      : "";
    this.filtrofecha = localStorage.getItem(this.id_user + "FACfiltrofecha")
      ? localStorage.getItem(this.id_user + "FACfiltrofecha")
      : "";
    this.filtrobaseimponible = localStorage.getItem(
      this.id_user + "FACfiltrobaseimponible"
    )
      ? localStorage.getItem(this.id_user + "FACfiltrobaseimponible")
      : "";
    this.filtroiva = localStorage.getItem(this.id_user + "FACfiltroiva")
      ? localStorage.getItem(this.id_user + "FACfiltroiva")
      : "";
    this.filtrototal = localStorage.getItem(this.id_user + "FACfiltrototal")
      ? localStorage.getItem(this.id_user + "FACfiltrototal")
      : "";
    this.filtrovencimiento = localStorage.getItem(
      this.id_user + "FACfiltrovencimiento"
    )
      ? localStorage.getItem(this.id_user + "FACfiltrovencimiento")
      : "";
    this.filtropedido = localStorage.getItem(this.id_user + "FACfiltropedido")
      ? localStorage.getItem(this.id_user + "FACfiltropedido")
      : "";
    this.filtrocobrada = localStorage.getItem(this.id_user + "FACfiltrocobrada")
      ? localStorage.getItem(this.id_user + "FACfiltrocobrada")
      : "";
    this.filtrofechacobrada = localStorage.getItem(
      this.id_user + "FACfiltrofechacobrada"
    )
      ? localStorage.getItem(this.id_user + "FACfiltrofechacobrada")
      : "";
    this.numPage = +(localStorage.getItem(this.id_user + "FACpage")
      ? localStorage.getItem(this.id_user + "FACpage")
      : 1);
  }
  ...
  filtrar() {
    ...
    let filtrofech: string =
      this.filtrofecha == undefined ? "" : this.filtrofecha;
    let filtrobi: string =
      this.filtrobaseimponible == undefined ? "" : this.filtrobaseimponible;
    let filtroiv: string = this.filtroiva == undefined ? "" : this.filtroiva;
    let filtrott: string =
      this.filtrototal == undefined ? "" : this.filtrototal;
    let filtrovto: string =
      this.filtrovencimiento == undefined ? "" : this.filtrovencimiento;
    let filtroped: string =
      this.filtropedido == undefined ? "" : this.filtropedido;
    let filtrocob: string =
      this.filtrocobrada == undefined ? "" : this.filtrocobrada;
    let filtrofcb: string =
      this.filtrofechacobrada == undefined ? "" : this.filtrofechacobrada;
    ...
    for (let i = 0; i < this.billings.length; i++) {
      if (
        this._common
          .getCleanedString(this.billings[i].numbertext)
          .indexOf(this._common.getCleanedString(filtronum)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].issue_date)
          .indexOf(this._common.getCleanedString(filtrofech)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].description)
          .indexOf(this._common.getCleanedString(filtrodesc)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].customer)
          .indexOf(this._common.getCleanedString(filtrocli)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].due_date)
          .indexOf(this._common.getCleanedString(filtrovto)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].ped_code)
          .indexOf(this._common.getCleanedString(filtroped)) >= 0 &&
        this._common
          .getCleanedString(this.billings[i].charge_date)
          .indexOf(this._common.getCleanedString(filtrofcb)) >= 0 &&
        this.billings[i].charge_amount.indexOf(
          this._common.getCleanedString(filtrocob)
        ) >= 0 // &&
  ...
  paginarysumar() {
    ...
    for (let i = 0; i < this.facturasfiltradas.length; i++) {
      ...
      this.totalbasesimponibles += parseFloat(
        this.facturasfiltradas[i].tax_base
          .toString()
          .replace(".", "")
          .replace(".", "")
          .replace(",", ".")
      );
      this.totalivas += parseFloat(
        this.facturasfiltradas[i].taxes
          .toString()
          .replace(".", "")
          .replace(".", "")
          .replace(",", ".")
      );
      this.totaltotales += parseFloat(
        this.facturasfiltradas[i].total
          .toString()
          .replace(".", "")
          .replace(".", "")
          .replace(",", ".")
  ...
  updateApi() {
    ...
    body += `&due_date=${this.selectedBill.due_date_model.date.year}-${this.selectedBill.due_date_model.date.month}-${this.selectedBill.due_date_model.date.day}`;
    this._api
      .updateBill(body)
  ...
  parseUpdate(response) {
    if (response.status === "ok") {
      ...
      this._notification.success("¡Éxito!", "Se ha actualizado la factura");
  ...
  updateRows(response) {
    this.billings[parseInt(response.idx)].description =
      response.item[0].description;
    ...
    this.billings[parseInt(response.idx)].due_date_no_parsed =
      response.item[0].due_date;
    this.billings[parseInt(response.idx)].due_date = this._common.getFecha(
      new Date(response.item[0].due_date)
    );
  }
    ...
    this.facturasfiltradas.forEach((line) => {
      if (this.id_company == 412) {
        exportData.push({
          ...
          Cobrado: this._common.getNumber(
            this._common.getDecimals(line.charge_amount, 2, true)
          ),
          "Fecha Cobro": line.charge_date,
          CIF:
            line.customer_cif == null
              ? ""
              : line.customer_cif.toString().trim(),
        });
      } else {
        exportData.push({
          Número: line.numbertext,
  ...
  fillExcelDataExportation(exportacion) {
    ...
    facturastodas.reverse().forEach((line) => {
      if (line.id_export == exportacion) {
        ...
        let fecha = new Date(
          fechatemp[0],
          parseInt(fechatemp[1]) - 1,
          fechatemp[2]
        );
        let bAbono =
          line.numbertext.toString().indexOf("AB") >= 0 ? true : false;
        let fecha_rect = null;
        if (line.issue_date_rect != 0)
          fecha_rect = line.issue_date_rect.split("-");
        ...
        if (dataCompany.id != 411) {
          exportData.push({
            ...
            "21base":
              line.percent_tax == 21 ? line.tax_base.replace(/\./g, "") : 0,
            "21cuota":
              line.percent_tax == 21
                ? (
                    Math.round(
            ...
            Total: (
              Math.round(
                (line.percent_tax == 21 ? 12100 : 10000) *
                  parseFloat(
                    line.tax_base
                      .toString()
                      .replace(".", "")
            ...
            exbase:
              line.percent_tax == 0 ? line.tax_base.replace(/\./g, "") : 0,
          ...
          exportData.push({
            ...
            FechaFacturaRectificada: fecha_rect
              ? fecha_rect[2] + "/" + fecha_rect[1] + "/" + fecha_rect[0]
              : "",
            BaseImponibleRectificada: bAbono
              ? this._common.getDecimals(line.tax_base_rect, 2, true)
              : "",
            CuotaIvaRectificada: bAbono
              ? this._common.getDecimals(line.taxes_rect, 2, true)
              : "",
            ...
            "Cod. Provincia":
              line.postal_code == 0 ? "" : line.postal_code.substring(0, 2),
            Provincia: line.city,
            CodigoCanal: "",
            CodigoDelegación: "",
            CodDepartamento: "",
            ...
            "Fecha Vencimiento":
              this._common.getFecha(fechavenc).indexOf("1899") >= 0
                ? ""
                : this._common.getFecha(fechavenc),
          });
        }
      }
    });
