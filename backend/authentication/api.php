...
/* 				$strQuery = $strQuery." order by campaign.campaign_code ASC"; */
		...
		private function addCampaign() {
			...
			if (isset($this->datosPeticion['campaign_code'], $this->datosPeticion['id_company'], $this->datosPeticion['id_fiscal_year'], $this->datosPeticion['campaign_name'], $this->datosPeticion['id_user'],
								...
								$this->datosPeticion['start_date_event'], $this->datosPeticion['end_date_event'])) {
				...
				$startDateEvent = $this->datosPeticion['start_date_event'];
				$endDateEvent = $this->datosPeticion['end_date_event'];
				$btramite = 0;
				...
				if($filasActualizadas === 0) {
					...
					$query = $this->_conn->prepare("insert into tt_campaign(id_company,id_fiscal_year,campaign_code,ped_code, campaign_name,id_user,id_customer,id_team,id_group,id_subgroup,id_status,creation_date, end_date, security_level,btramite,start_date_event, end_date_event) values(
						'".$id_company."','".$id_fiscal_year."','".$campaign_code."','".$ped_code."','".$campaign_name."', '".$id_user."','".$id_customer."','".$id_team."','".$id_group."','".$id_subgroup."','".$id_status."','".$creation_date."','".$end_date."','".$id_security."','".$btramite."','".$startDateEvent."','".$endDateEvent."')");
					$query->execute();
					$filasActualizadas = $query->rowCount();
					if ($filasActualizadas == 1) {
						$respuesta['status'] = 'ok';
...
/* 					if($percent['percent_tax'] == 0) {
	...
	private function updateObservations() {
		...
		if (isset($this->datosPeticion['id'],$this->datosPeticion['observ_cli'], $this->datosPeticion['observ_int'])) {
			...
			$shipping_method = html_entity_decode($this->datosPeticion['shipping_method']);
			$shipping_method_return = html_entity_decode($this->datosPeticion['shipping_method_return']);


			if (!empty($id)) {

				$query = $this->_conn->prepare("update tt_campaign set shipping_method='".$shipping_method."',  shipping_method_return='".$shipping_method_return."', observ_cli='".$observ_cli."', observ_int='".$observ_int."' where id='".$id."'");
 ...
 */
			...
			art.id_customer, customer_name, contact, art.image, 
			art.ref_provider, art.descrip_provider, art.tipo_torno
			FROM tt_subconcepts_standards art
			left join tt_articles_envelopes env
			on env.id = art.envelope_type
			left join tt_articles_families fam
			on fam.id = art.id_family
...
/* 				$response = $this->operationApi("SELECT * FROM tt_fee_company where id_bill='".$idBill."'", 'GET');
	...
	private function getSubconcept() {
		if (isset($this->datosPeticion['id_company'], $this->datosPeticion['id_fiscal_year']) ) {
			...
			if ( $response != false) {
				...
				if ($id_project) {
					$response = $this->operationApi("SELECT id_status,start_date_event,end_date_event, start_date_budget, end_date_budget, budget_validity, campaign_code, campaign_name FROM tt_campaign where id='".$id_project."'", 'GET');
					if($response!=false) {
	...
	public function getInfoCampaign() {
	  ...
	  if (isset($this->datosPeticion['id'], $this->datosPeticion['id_company'], $this->datosPeticion['id_fiscal_year'])) {
			...
			campaign.id_address, campaign.contact, campaign.shipping_method, campaign.shipping_method_return,
